...
description: Step-by-step guide to deploying VoxLibris — environment variables, database, API keys, and TTS engines
category: Setup
order: 0
...
This guide walks through everything needed to run VoxLibris from scratch: obtaining an API key, configuring the environment, deploying the application, and connecting a TTS engine.

---

## 1. Obtain an OpenRouter API Key

VoxLibris uses a language model for two purposes:
...
VoxLibris is configured through environment variables. The simplest approach is a `.env` file in the project root.
...
| `APP_PORT` | No | Host port VoxLibris listens on (default: `5000`) |
...
The bundled `docker-compose.yml` starts both a Postgres container and the VoxLibris app container. This is the easiest way to get everything running.
...
VoxLibris ships with two built-in engines that require no setup:
...
### Step 7 — Register the engine in VoxLibris

1. Open VoxLibris and go to **Settings → Remote TTS Engines**
2. Click **Add Engine**
3. Fill in:
...
VoxLibris will call `POST /GetEngineDetails` on the Space, verify connectivity, and register the engine. It appears in the engine table with its status and voice count. If the Space is still building or warming up, VoxLibris will show it as **warming up** and poll automatically until it's ready.
...
- Confirm the API Token in VoxLibris matches the `API_KEY` secret in the Space exactly
